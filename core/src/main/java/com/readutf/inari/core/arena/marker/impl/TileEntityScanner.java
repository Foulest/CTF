...
public class TileEntityScanner implements MarkerScanner {
    ...
    private static final Logger logger = LoggerFactory.getLogger(TileEntityScanner.class);

    @Override
    public List<Marker> scan(@NotNull WorldCuboid worldCuboid) {
        ...
        System.out.println("Scanning " + chunks.size() + " chunks for tile entities...");
        ...
        for (Chunk chunk : chunks.values()) {
            ...
            for (BlockState tileEntity : chunk.getTileEntities()) {
                if (tileEntity instanceof Sign) {
                    if (!worldCuboid.contains(tileEntity.getLocation())) {
                        continue;
                    }
