...
public class GridArenaManager extends ArenaManager {
    ...
    @Getter
    private static final World activeArenasWorld;

    static {
        // Create the template world
        ...
        if (templateArenaWorld != null) {
            templateArenaWorld.setAutoSave(false);
        }

        // Create the active arenas world
    ...
    public GridArenaManager(JavaPlugin javaPlugin, File baseDir, MarkerScanner markerScanner) {
        ...
        arenasFolder.mkdirs();
        gridPositionManager = new GridPositionManager(1000);
        availableArenas = loadAvailableArenas();
        File file = new File(baseDir, "grid_settings.json");

        if (file.exists()) {
            gridSettings = gson.fromJson(new FileReader(file, StandardCharsets.UTF_8), GridSettings.class);
        } else {
    ...
    @Override
    protected void save(@NotNull WorldCuboid worldCuboid, Arena arena) {
        ...
        int startChunkMinX = (int) (min.getX() - (minChunk.getX() << 4));
        int startChunkMinZ = (int) (min.getZ() - (minChunk.getZ() << 4));
    ...
    public CompletableFuture<ActiveArena> load(ArenaMeta arenaMeta) throws ArenaLoadException {
        ...
        if (!bufferedArenas.isEmpty()) {
            ...
            CompletableFuture.runAsync(() -> Bukkit.getScheduler().scheduleSyncDelayedTask(javaPlugin, () -> {
                try {
                    spawnNewArena(arenaMeta);
                } catch (ArenaLoadException ex) {
                    ex.printStackTrace();
                }
            }, 1));
    ...
    public ActiveArena spawnNewArena(@NotNull ArenaMeta arenaMeta) throws ArenaLoadException {
        ...
        try {
            ...
            int endChunkX = (max.getBlockX() >> 4) + 1;
            int endChunkZ = (max.getBlockZ() >> 4) + 1;
            int targetMinPosX = (gridX << 4) + ((startChunkX << 4) + min.getBlockX());
            int targetMinPosZ = (gridZ << 4) + ((startChunkZ << 4) + min.getBlockZ());

            logger.info("Creating relighter...");
            Relighter relighter = createRelighter();

            logger.info("Copying chunks..."); // NOTE: Also adds chunks to relighter
            Iterable<Chunk> chunks = enqueueRelightAndChunkPaste(startChunkX, endChunkX, startChunkZ, endChunkZ, minChunk, relighter, gridX, gridZ);
            ...
            throw new ArenaLoadException(ex.getMessage(), ex);
        }
    }
    ...
    private static Relighter createRelighter() {
        ...
        return platform.getRelighterFactory().createRelighter(RelightMode.OPTIMAL, weWorld, Fawe.instance().getQueueHandler().getQueue(weWorld));
    }

    private static @NotNull Iterable<Chunk> enqueueRelightAndChunkPaste(
            int startChunkX, int endChunkX, int startChunkZ, int endChunkZ,
            Chunk minChunk, Relighter relighter, int gridX, int gridZ) {
        Deque<Chunk> toPaste = new ArrayDeque<>();
