...
public class TeamBasedSpawning implements SpawnFinder {
    ...
    public TeamBasedSpawning(@NotNull Game game, String prefix) {
        this.game = game;
        this.prefix = prefix;
        logger = game.getLoggerFactory().getLogger(TeamBasedSpawning.class);
    }

    @Override
    public @NotNull Location findSpawn(@NotNull Player player) throws GameException {
    ...
    private final HashMap<ActiveArena, HashMap<Integer, List<Location>>> teamSpawnsCache = new HashMap<>();

    private @NotNull HashMap<Integer, List<Location>> getTeamSpawns(ActiveArena activeArena) {
