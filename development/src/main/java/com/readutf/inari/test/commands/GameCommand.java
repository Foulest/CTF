...
public class GameCommand extends BaseCommand {
    ...
    @SneakyThrows
    @Subcommand("start")
    @CommandCompletion("@gametypes @arena @players")
    public void start(Player player, String gameName, String arenaName, String @NotNull ... players) {
        if (players.length % 2 != 0) {
            player.sendMessage("Teams must be the same size.");
            return;
        }
        ...
        GameStarter gameStarter = gameStarterManager.getStarter(gameName.toLowerCase(Locale.ROOT));
