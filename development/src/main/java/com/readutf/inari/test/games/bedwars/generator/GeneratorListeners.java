...
public class GeneratorListeners {
    ...
    public GeneratorListeners(GeneratorManager generatorManager, Game game) {
        ...
        pickupHistory = new EnumMap<>(Material.class);
    }

    @GameEventHandler
    public void onItemPickup(@NotNull EntityPickupItemEvent event) {
        Generator generator = generatorManager.getNearbyGenerator(event.getItem().getItemStack(), event.getItem().getLocation());

        if (generator != null) {
            System.out.println("Item pickup " + event.getItem().getItemStack().getAmount());
    ...
    private static @NotNull List<Integer> getItemAmounts(int stackSize, int numOfPeople) {
